...
def user_register_f(request: Request) -> Response:
    ...
    # curl -v -X POST 127.0.0.1:8000/api/token/ -d '{"username": "admin@gmail.com", "password": "31284bogdan"}'
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = django_serializers.UserSerializer


class GroupViewSet(viewsets.ModelViewSet):
    queryset = Group.objects.all()
    serializer_class = django_serializers.GroupSerializer
...
def post_create_f(request: Request) -> Response:
    ...
    if len(request.data["title"]) < 3:
        return Response(data="Incorrect title", status=status.HTTP_400_BAD_REQUEST)
